...
가비지 컬렉션은 메모리를 관리하는 방법중에 하나로 Young 영역에서 발생하는 GC를 Minor GC Old 영역에서 발생하는 GC를 Major GC라고 한다. 이러한 GC의 종류에는 Serial GC, Parallel Young GC, Concurrent Mark & Sweep Collector (줄여서 CMS), G1 GC가 있다.<br>
- Serial GC<br>
